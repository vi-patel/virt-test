...
def analyze_results(file_path, test_type):
    ...
    if "type_and_func_keys" == test_type:
    ...
    for i in range(len(correct_keycodes)):
        if not (test_keycodes[i] == correct_keycodes[i]):
            return "Testing of sending keys failed:" + \
                   "  Expected keycode = %s" % correct_keycodes[i]
...
def run_rv_input(test, params, env):
    ...
    Test depends on rv_conenct test.
    ...
    if "Red Hat" in guest_session.cmd("cat /etc/redhat-release"):
    ...
    if "type_and_func_keys" == test_type:
        test_type_and_func_keys(client_vm, guest_session, params)
    elif "leds_and_esc_keys" == test_type:
        test_leds_and_esc_keys(client_vm, guest_session, params)
    elif "nonus_layout" == test_type:
        test_nonus_layout(client_vm, guest_session, params)
    elif "leds_migration" == test_type:
        test_leds_migration(client_vm, guest_vm, guest_session, params)
    else:
