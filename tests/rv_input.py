...
def deploy_epel_repo(guest_session, params):
    ...
    if "NeedsInstall" in output:
        ...
        if "release 5" in guest_session.cmd("cat /etc/redhat-release"):
            cmd = ("yum -y localinstall http://download.fedoraproject.org/pub/epel/5/"
                  "%s/epel-release-5-4.noarch.rpm 2>&1" % arch)
            ...
            cmd = ("yum -y localinstall http://download.fedoraproject.org/pub/epel/6/"
                  "%s/epel-release-6-8.noarch.rpm 2>&1" % arch)
            logging.info("Installing epel repository to %s",
                        params.get("guest_vm"))
            guest_session.cmd(cmd, print_func=logging.info, timeout=90)
        else:
